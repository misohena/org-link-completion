...
org-link-completion-at-point関数は、ポイントがある場所の周辺を解析したあと、ポイントがリンク内のどの部位にあるかやリンクタイプ(<type>)が何かによって呼び出す関数を次のように決定します。

- ポイントが<type>の部分にある => org-link-completion-capf-type 変数(デフォルト: org-link-completion-capf-type-default 関数)

- <type>が空:
  - ポイントが<path>の部分にある => org-link-completion-capf-path-untyped 変数(デフォルト: org-link-completion-capf-path-untyped-default 関数)
  - ポイントが<desc>の部分にある => org-link-completion-capf-desc-untyped 変数(デフォルト: org-link-completion-capf-desc-untyped-default 関数)

- <type>が有効なリンクタイプ(org-link-parameters変数に含まれている):
  org-link-parameters 変数に設定されている、リンクタイプ<type>の次のプロパティから関数を取得してそれを呼び出す:
...
- <type>に対する補完関数が見つからない:
  - ポイントが<path>の部分にある => org-link-completion-capf-path-unknown-type 変数(デフォルト: org-link-completion-capf-path-unknown-type-default 関数)
  - ポイントが<desc>の部分にある => org-link-completion-capf-desc-unknown-type 変数(デフォルト: org-link-completion-capf-desc-unknown-type-default 関数)

org-link-completion-at-point関数から呼び出される関数には引数は渡されません。ただし、解析した情報を org-link-completion-pos 変数にキャッシュしてから呼び出します。呼び出された関数は必要に応じてその情報を参照するか、無視して再解析することも出来ます。
...
org-link-completion-at-point関数から呼び出される関数は通常この関数を直接呼び出す必要はありません。org-link-completion-pos変数にキャッシュされた結果が格納されているのでそこから取り出すことが出来ます。ただし、キャッシュされた値がない場合に備えたコードを書くことも可能です。
