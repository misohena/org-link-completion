...
このEmacs Lispライブラリは、org-modeのリンク表記内で completion-at-point (M-TAB、C-M-i、またはESC TAB) できるようにします。

ポイントがある場所を解析し、リンク内の部位やリンクタイプに応じて適切な関数を呼び出すための拡張可能な枠組みを提供します。

デフォルトで提供している補完は次の通りです。

- リンクタイプ部分
- ~file:~ (または ~file+sys:~ ~file+emacs:~)リンクタイプのパス部分と説明部分

他の部分はユーザーが補完関数を追加する必要があります。

このライブラリを使用した補完関数を提供する別のライブラリとして次のものがあります。
